...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, SparseD_param=SparseD_param)
        ...
        if self.config.block_group_size == 1:
            ...
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, SparseD_param=SparseD_param)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
            # print(f"Now-{now_step}/{new_generation}: time {time.time()-start_time:.4f}")
        else:
